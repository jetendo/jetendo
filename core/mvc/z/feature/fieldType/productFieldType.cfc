...
<cffunction name="getFormField" localmode="modern" access="public">
	...
	<script>
		...
		zArrDeferredFunctions.push(function(){
			<cfloop query="qProd">
				...
				<cfif Trim(sValue) EQ qProd.product_id>
					...
					$("###arguments.prefixString##arguments.row["feature_field_id"]#").append("<option selected data-category=\"#qProd.product_category_id#\" value=\"#qProd.product_id#\">#qProd.product_name#</option>");
				</cfif>
			</cfloop>
		});
		function pspMgrProdChanged_#arguments.row["feature_field_id"]#(ctrl){
			var $lstCtrl = $("##lstProductName");
			$("##product_name_#arguments.row["feature_field_id"]#").val($("###arguments.prefixString##arguments.row["feature_field_id"]# option:selected").text());
			var sCategory = $("###arguments.prefixString##arguments.row["feature_field_id"]# option:selected").attr("data-category");
		...
		function pspMgrHideList_#arguments.row["feature_field_id"]#(ctrl, lstCtrl) {
			var $theCtrl = $("##" + lstCtrl);
		...
		function pspMgrSearchProduct_#arguments.row["feature_field_id"]#(e, ctrl) {
			if(e.keyCode == 27)
				return false;
			$("###arguments.prefixString##arguments.row["feature_field_id"]#").empty();
			$("###arguments.prefixString##arguments.row["feature_field_id"]#").append("<option value=\"\"></option>");
			...
			for(var idx in arrProdInSpecial){
				...
				if(searchText == "" || prod.id.indexOf(searchText) != -1 || prod.name.toLowerCase().indexOf(searchText) != -1){
					$("###arguments.prefixString##arguments.row["feature_field_id"]#").append("<option data-category=\"" + prod.category + "\" value=\""+ prod.id + "\">" + prod.name + "</option>");
				}
			}
			var $lstCtrl = $("##lstProductName");
	...
	<input style="width:200px; height:25px;" type="text" id="product_name_#arguments.row["feature_field_id"]#" value="#productName#"
		name="product_name_#arguments.row["feature_field_id"]#" list="productList" onkeyup="pspMgrSearchProduct_#arguments.row["feature_field_id"]#(event, this);" />
	<button type="button" id="ddBtnProductName" onclick="pspMgrHideList_#arguments.row["feature_field_id"]#(this,'lstProductName');" style="padding:0px; width:22px; height:24px; top:1px; position:relative; left:-3px; cursor:pointer;">▼</button>
	<datalist id="lstProductName" style="display:none; height:300px; width:250px; padding-left:0px;">
		<select class="zProductIdClass" name="#arguments.prefixString##arguments.row["feature_field_id"]#" id="#arguments.prefixString##arguments.row["feature_field_id"]#" size="8" style="width:350px;" onchange="pspMgrProdChanged_#arguments.row["feature_field_id"]#(this);">
...
<cffunction name="getTypeForm" localmode="modern" access="public">
	...
	<cfsavecontent variable="output">
		<input type="radio" name="feature_field_type_id" value="25" onClick="setType(25);" <cfif value EQ 25>checked="checked"</cfif>/>
		#this.getTypeName()#<br />
	</cfsavecontent>
	<cfreturn output>
</cffunction> 
