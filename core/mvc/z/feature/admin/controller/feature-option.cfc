...
<cffunction name="updateAllSitesFieldCache" access="public" localmode="modern">  
	<cfscript>
	db=request.zos.queryObject;
	db.sql="SELECT site_id FROM #db.table("site", "jetendofeature")# 
	WHERE site_id <>#db.param(-1)# and 
	site_active=#db.param(1)# and 
	site_deleted = #db.param(0)#";
	qS=db.execute("qS"); 
	for(row in qS){
		updateFieldCache(row.site_id);
	} 
	</cfscript>
</cffunction>
...
<cffunction name="internalUpdateSchemaCacheBySchemaId" access="public" localmode="modern">
	...
	for(row in qSchema){
		...
		sog.optionSchemaIdLookup[row.feature_schema_parent_id&chr(9)&row.feature_schema_name]=row.feature_schema_id;
	}
	...
	if(versioningEnabled or cacheEnabled){
		db.sql="SELECT s1.* 
		FROM #db.table("site_x_option_group_set", "jetendofeature")# s1   
		...
		for(row in qS){
			...
			if(structkeyexists(sog.optionSchemaSetId, id) EQ false){
				if(structkeyexists(sog.optionSchemaSetId, id&"_appId") EQ false){
					...
					sog.optionSchemaSetId[id&"_parentId"]=row.site_x_option_group_set_parent_id;
					sog.optionSchemaSetId[id&"_childSchema"]=structnew();
				}
				if(structkeyexists(sog.optionSchemaSetId, row.site_x_option_group_set_parent_id&"_childSchema")){
					if(structkeyexists(sog.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"], row.feature_schema_id) EQ false){
						sog.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id]=arraynew(1);
					}
					...
					if(structkeyexists(tempUniqueStruct, row.site_x_option_group_set_parent_id&"_"&id) EQ false){ 
						arrayappend(sog.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id], id);
		...
		if(cacheEnabled or (versioningEnabled and arraylen(arrVersionSetId))){
			db.sql="SELECT s3.site_x_option_group_set_id, s3.feature_field_id groupSetFieldId, 
		 ...
		 db.sql="SELECT * FROM 
		 #db.table("site_x_option_group_set", "jetendofeature")# s1, 
		...
		for(row in qS){
			if(cacheEnabled){
				sog.optionSchemaSetQueryCache[row.site_x_option_group_set_id]=row;
			}
			if(structkeyexists(sog.optionSchemaSetArrays, row.feature_id&chr(9)&row.feature_schema_id&chr(9)&row.site_x_option_group_set_parent_id) EQ false){
				sog.optionSchemaSetArrays[row.feature_id&chr(9)&row.feature_schema_id&chr(9)&row.site_x_option_group_set_parent_id]=arraynew(1);
			}
			ts=structnew();
			...
			if(row.feature_schema_enable_unique_url EQ 1){
				if(row.site_x_option_group_set_override_url NEQ ""){
					...
					var urlId=arguments.siteStruct.optionSchemaUrlId;
			...
			sog.optionSchemaSet[row.site_x_option_group_set_id]= ts;


			if(row.site_x_option_group_set_master_set_id NEQ 0){
				if(structkeyexists(sog.optionSchemaSet, row.site_x_option_group_set_master_set_id)){
					masterStruct=sog.optionSchemaSet[row.site_x_option_group_set_master_set_id];
					ts.__sort=masterStruct.__sort;
					if(row.feature_schema_enable_unique_url EQ 1){
						ts.__url=masterStruct.__url;
					}
				}
				sog.optionSchemaSetVersion[row.site_x_option_group_set_master_set_id]=ts.__setId;
			}else{
				arrayappend(sog.optionSchemaSetArrays[row.feature_id&chr(9)&ts.__groupId&chr(9)&row.site_x_option_group_set_parent_id], ts);
			}
		} 
	}
...
<cffunction name="internalUpdateFieldFieldCache" access="public" localmode="modern">
	<cfargument name="siteStruct" type="struct" required="yes">
	<cfscript>
	db=request.zos.queryObject;
	site_id=arguments.siteStruct.id;
	...
	feature_field.site_id = if(site_x_option.feature_field_id_siteIDType = #db.param(1)#, #db.param(site_id)#, 
		if(site_x_option.feature_field_id_siteIDType = #db.param(4)#, #db.param(0)#, #db.param(site_id)#)) and 
	site_x_option_deleted = #db.param(0)#
	WHERE feature_field.site_id IN (#db.param('0')#,#db.param(site_id)#) and 
	feature_field_deleted = #db.param(0)# and 
	feature_schema_id = #db.param(0)#"; 
	qS=db.execute("qS"); 
	tempStruct={}; 
	tempStruct.feature_fields=structnew();
	tempStruct.feature_field_edit_enabled=structnew();
	tempStruct.feature=structnew();
	for(row in qS){
		tempStruct.feature_field_edit_enabled[row.feature_field_name]=row.feature_field_edit_enabled;
		if(row.feature_field_type_id EQ 1 and row.feature_field_line_breaks EQ 1){
			if(row.site_x_option_id EQ ""){
				c1=application.zcore.functions.zparagraphformat(row.feature_field_default_value);
			}else{
				c1=application.zcore.functions.zparagraphformat(row.site_x_option_value);
			}
		}else{
			if(row.site_x_option_id EQ ""){
				c1=row.feature_field_default_value;
			}else{
				c1=row.site_x_option_value;
			}
			if(c1 NEQ "" and c1 NEQ "0" and (row.feature_field_type_id EQ 3 or row.feature_field_type_id EQ 9)){
				c1="/zupload/site-options/"&c1;
			}
		}
		if(row.feature_id NEQ 0){
			if(structkeyexists(tempStruct.feature, row.feature_id) EQ false){
				tempStruct.feature[row.feature_id]=structnew();
			}
			tempStruct.feature[row.feature_id][row.feature_field_name]=c1;
			if(row.site_x_option_original NEQ ""){
				tempStruct.feature[row.feature_id]["__original "&row.feature_field_name]="/zupload/site-options/"&row.site_x_option_original;
			}else{
				tempStruct.feature[row.feature_id]["__original "&row.feature_field_name]=c1;
			}
		}else{
			tempStruct.feature_fields[row.feature_field_name]=c1;
			if(row.feature_field_type_id EQ 3){
				if(row.site_x_option_original NEQ ""){
					tempStruct.feature_fields["__original "&row.feature_field_name]="/zupload/site-options/"&row.site_x_option_original;
				}else{
					tempStruct.feature_fields["__original "&row.feature_field_name]=c1;
				}
			}
		}
	}
...
<cffunction name="internalUpdateFieldAndSchemaCache" access="public" localmode="modern">
	...
	//tempStruct.soSchemaData.optionLookup={};
	//tempStruct.soSchemaData.optionIdLookup={};
	//sog=tempStruct.soSchemaData;
	
	updateSchemaCache(tempStruct);
	internalUpdateFieldFieldCache(tempStruct);
...
<cffunction name="setURLRewriteStruct" localmode="modern" access="public">
	...
	if(structkeyexists(request.zos.globals,'optionSchemaURLID') and request.zos.globals.optionSchemaURLID NEQ 0){
		ts2.reservedAppUrlIdStruct[request.zos.globals.optionSchemaURLid]=[];
		t9=structnew();
		t9.type=1;
		...
		arrayappend(ts2.reservedAppUrlIdStruct[request.zos.globals.optionSchemaURLid], t9);
		...
		if(request.zos.enableSiteSchemaCache and groupStruct.feature_schema_enable_cache EQ 1){
			...
			if(arguments.orderBy NEQ ""){
				...
				if(structkeyexists(optionIdLookup, groupId&chr(9)&arguments.orderBy)){
					...
					throw("arguments.orderBy, ""#arguments.orderBy#"" is not a valid field in the feature_schema_id=#groupId# | ""#groupStruct.feature_schema_name#""");
				}
...
<cffunction name="optionSchemaSetFromDatabaseBySearch" access="public" returntype="array" localmode="modern">
	...
	if(not structkeyexists(ts, 'arrSchemaName')){
		throw("arguments.searchStruct.arrSchemaName is required. It must be an array of feature_schema_name values.");
	}
...
<cffunction name="getSetParentLinks" access="public" localmode="modern">
	...
	if(arguments.linkCurrentPage){
		if(form.method NEQ "sectionSchema"){
			arrayAppend(arrParent, '<a href="/z/feature/admin/features/sectionSchema?site_x_option_group_set_id=#form.site_x_option_group_set_id#">Manage Section</a> /');
		}
		manageAction="manageSchema";
		if(form.method EQ "userManageSchema"){
			manageAction="userManageSchema";
		}
		arrayAppend(arrParent, '<a href="/z/feature/admin/features/#manageAction#?feature_schema_id=#curSchemaId#&amp;site_x_option_group_set_parent_id=#curParentSetId#">Manage #groupStruct.feature_schema_name#(s)</a> / ');
	}
	if(curParentSetId NEQ 0){
		loop from="1" to="25" index="i"{
			...
			loop query="q12"{
				...
				out='<a href="#application.zcore.functions.zURLAppend("/z/feature/admin/features/#manageAction#", "feature_schema_id=#q12.feature_schema_id#&amp;site_x_option_group_set_parent_id=#q12.d3#")#">#application.zcore.functions.zFirstLetterCaps(q12.feature_schema_display_name)#</a> / ';
				if(not arguments.linkCurrentPage and curSchemaID EQ arguments.feature_schema_id){
					...
					out&='<a href="/z/feature/admin/features/#manageAction#?feature_schema_id=#curSchemaId#&amp;site_x_option_group_set_parent_id=#q12.d2#">#application.zcore.functions.zLimitStringLength(application.zcore.functions.zRemoveHTMLForSearchIndexer(q12.site_x_option_group_set_title), 70)#</a> /';
				}
...
<cffunction name="optionSchemaSetFromDatabaseBySetId" access="public" returntype="struct" localmode="modern">
	...
	db=request.zos.noVerifyQueryObject;
	 db.sql="SELECT * FROM 
	 #db.table("site_x_option_group_set", "jetendofeature")# s1 FORCE INDEX(`PRIMARY`), 
...
<cffunction name="optionSchemaSetFromDatabaseBySchemaId" access="public" localmode="modern">
	...
	for(row in qS){
		...
		variables.buildSchemaSetIdField(row, rowStruct[row.site_x_option_group_set_id]);
...
<cffunction name="buildSchemaSetId" access="private" localmode="modern">
	...
	if(groupStruct.feature_schema_enable_unique_url EQ 1){
		if(row.site_x_option_group_set_override_url NEQ ""){
			...
			var urlId=application.zcore.functions.zvar('optionSchemaURLID', row.site_id);
			if(urlId EQ "" or urlId EQ 0){
				throw("feature_schema_url_id is not set for site_id, #arguments.site_id#.");
			}
			ts.__url="/#application.zcore.functions.zURLEncode(row.site_x_option_group_set_title, '-')#-#urlId#-#row.site_x_option_group_set_id#.html";
		}
	}
...
<cffunction name="setSchemaImportStruct" access="public" localmode="modern">
	...
	form.feature_schema_id=groupId;//optionSchemaIDByName(arguments.feature_schema_name, arguments.feature_schema_parent_id);
...
<cffunction name="resortSchemaSets" localmode="modern" access="public">
	...
	for(var row2 in qSort){
		...
		if(structkeyexists(groupStruct, 'feature_schema_change_cfc_path') and groupStruct.feature_schema_change_cfc_path NEQ ""){
			...
			changeCom[groupStruct.feature_schema_change_cfc_sort_method](row2.site_x_option_group_set_id, i);
		}
		i++;
	}
	t9=getSiteData(arguments.site_id);
	t9.optionSchemaSetId[arguments.site_x_option_group_set_parent_id&"_childSchema"][arguments.feature_schema_id]=arrTemp;

	arrData=t9.optionSchemaSetArrays[arguments.feature_id&chr(9)&arguments.feature_schema_id&chr(9)&arguments.site_x_option_group_set_parent_id];
	arrDataNew=[];
	for(i=1;i LTE arraylen(arrData);i++){
		sortIndex=sortStruct[arrData[i].__setId];
		arrDataNew[sortIndex]=arrData[i];
	}
	t9.optionSchemaSetArrays[arguments.feature_id&chr(9)&arguments.feature_schema_id&chr(9)&arguments.site_x_option_group_set_parent_id]=arrDataNew;
	</cfscript>
</cffunction>
...
<cffunction name="updateSchemaSetIdCache" localmode="modern" access="public">
	...
	for(row in qS){
		...
		if(structkeyexists(t9.optionSchemaSetId, id&"_appId") EQ false){
			...
			t9.optionSchemaSetId[id&"_parentId"]=row.site_x_option_group_set_parent_id;
			t9.optionSchemaSetId[id&"_childSchema"]=structnew();
		}
		if(row.site_x_option_group_set_master_set_id EQ 0 and structkeyexists(t9.optionSchemaSetId, row.site_x_option_group_set_parent_id&"_childSchema")){
			if(structkeyexists(t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"], row.feature_schema_id) EQ false){
				t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id]=arraynew(1);
			}
			if(typeStruct.optionSchemaLookup[row.feature_schema_id].feature_schema_enable_sorting EQ 1){
				if(structkeyexists(tempUniqueStruct, row.site_x_option_group_set_parent_id&"_"&id) EQ false){
					var arrChild=t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id];
					var resort=false;
			...
			//writedump(resort);
					if(resort){
						...
						t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id]=arrTemp;
					}
					//writedump(t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id]);
					tempUniqueStruct[row.site_x_option_group_set_parent_id&"_"&id]=true;
				}
			}else if(newRecord){
	...
	for(row in qS){
		...
		if(request.zos.enableSiteSchemaCache and row.feature_schema_enable_cache EQ 1){
			t9.optionSchemaSetQueryCache[row.site_x_option_group_set_id]=row;
		}
		if(structkeyexists(t9.optionSchemaSetArrays, row.feature_id&chr(9)&row.feature_schema_id&chr(9)&row.site_x_option_group_set_parent_id) EQ false){
			t9.optionSchemaSetArrays[row.feature_id&chr(9)&row.feature_schema_id&chr(9)&row.site_x_option_group_set_parent_id]=arraynew(1);
		}
		var ts=structnew();
		ts.__sort=row.site_x_option_group_set_sort;
		ts.__setId=row.site_x_option_group_set_id;
		...
		if(row.feature_schema_enable_unique_url EQ 1){
			if(row.site_x_option_group_set_override_url NEQ ""){
				...
				var urlId=application.zcore.functions.zvar('optionSchemaUrlID', arguments.site_id);
				if(urlId EQ "" or urlId EQ 0){
					throw("feature_schema_url_id is not set for site_id, #arguments.site_id#.");
				}
				ts.__url="/#application.zcore.functions.zURLEncode(row.site_x_option_group_set_title, '-')#-#urlId#-#row.site_x_option_group_set_id#.html";
		...
		t9.optionSchemaSet[row.site_x_option_group_set_id]= ts;
		arrChild=[];

		// don't sort versions
		if(row.site_x_option_group_set_master_set_id EQ 0){
			if(typeStruct.optionSchemaLookup[row.feature_schema_id].feature_schema_enable_sorting EQ 1){
				var arrChild=t9.optionSchemaSetArrays[row.feature_id&chr(9)&ts.__groupId&chr(9)&row.site_x_option_group_set_parent_id];
				var resort=false;
				...
				if(resort){  
					...
					if(not structkeyexists(t9.optionSchemaSetId, row.site_x_option_group_set_parent_id&"_childSchema")){
						t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"]={};
					}
					if(not structkeyexists(t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"], row.feature_schema_id)){
						t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id]=[];
					}
					try{
						var arrChild2=t9.optionSchemaSetId[row.site_x_option_group_set_parent_id&"_childSchema"][row.feature_schema_id];
						var arrTemp=[]; 
						for(var i=1;i LTE arraylen(arrChild2);i++){
							arrayAppend(arrTemp, t9.optionSchemaSet[arrChild2[i]]);
						}
						t9.optionSchemaSetArrays[row.feature_id&chr(9)&ts.__groupId&chr(9)&row.site_x_option_group_set_parent_id]=arrTemp;
					}catch(Any e){
...
<cffunction name="getSiteMap" localmode="modern" access="public">
	...
	qSchema=db.execute("qSchema");
	for(row in qSchema){ 
		arr1=optionSchemaStruct(row.feature_schema_name, 0, row.site_id, {__groupId=0,__setId=0}, row.feature_field_name);
...
<cffunction name="searchReindex" localmode="modern" access="public" hint="Reindex ALL site-option records in the entire app.">
	...
	db.sql="select feature_schema_id, feature_schema_parent_id, feature_schema_name, site_id FROM
	...
	for(row in qSchema){
		...
		groupStruct[row.site_id][row.feature_schema_id]={
			...
			name:row.feature_schema_name
		};
	}
	while(true){
		db.sql="select site_x_option_group_set_id, feature_schema.feature_schema_parent_id, site.site_id, feature_schema.feature_schema_name FROM
		...
		if(qSchema.recordcount EQ 0){
			...
			for(row in qSchema){
				...
				arrayAppend(arrSchema, row.feature_schema_name);
				indexSchemaRow(row.site_x_option_group_set_id, row.site_id, arrSchema); 
			}
		}
	}
...
<cffunction name="deactivateSchemaSet" localmode="modern" access="public">
	...
	db.execute("qUpdate");
	db.sql="select feature_schema_id, site_x_option_group_set_image_library_id from #db.table("site_x_option_group_set", "jetendofeature")# 
	WHERE site_id =#db.param(arguments.site_id)# and 
	site_x_option_group_set_deleted = #db.param(0)# and 
	site_x_option_group_set_id = #db.param(arguments.setId)# ";
	qSet=db.execute("qSet");
	if(qSet.recordcount){
		...
		deleteSchemaSetIndex(qSet.site_x_option_group_set, qSet.site_id);
...
<cffunction name="prepareRecursiveData" localmode="modern" access="public">
	...
	if(structkeyexists(ts,'selectmenu_groupid') and ts.selectmenu_groupid NEQ ""){
		...
		found=false; 
		if(qParentSchema.recordcount NEQ 0 and qParentSchema.feature_schema_parent_id NEQ 0 and form.site_x_option_group_set_parent_id NEQ 0){ 
			for(i=1;i<=50;i++){
		...
		if(qTemp.recordcount NEQ 0){
			...
			//	db.sql&=", s3.site_x_option_group_value parentId ";
			 ...
			 db.sql&=" from (
			 #db.table("site_x_option_group_set", "jetendofeature")# set1,
...
<cffunction name="deleteFieldAppId" localmode="modern" returntype="any" output="no">
	...
	if(arguments.feature_id NEQ 0 and arguments.feature_id NEQ ""){
		...
		site_id = #db.param(arguments.site_id)#";
		q=db.execute("q");
		db.sql="DELETE FROM #request.zos.queryObject.table("site_x_option_group_set", "jetendofeature")#  
		WHERE feature_id = #db.param(arguments.feature_id)# and 
		site_x_option_group_set_deleted = #db.param(0)# and 
		site_id = #db.param(arguments.site_id)#";
		q=db.execute("q");
...
<cffunction name="getEditableSchemaSetById" localmode="modern" access="public">
	...
	<cfscript>
	var s=getSchemaSetById(arguments.arrSchemaName, arguments.site_x_option_group_set_id);
	var db=request.zos.queryObject;
...
<cffunction name="deleteSchemaSetRecursively" localmode="modern" access="public" roles="member">
	...
	//writeLogEntry("deleteSchemaSetRecursively set id:"&arguments.site_x_option_group_set_id);
	db.sql="SELECT * FROM #db.table("site_x_option_group_set", "jetendofeature")# 
	...
	for(row2 in qSets){
		deleteSchemaSetRecursively(row2.site_x_option_group_set_id, {});
	}
	if(row.site_x_option_group_set_image_library_id NEQ 0){
	...
	if(row.site_x_option_group_set_master_set_id EQ 0){
		...
		for(row2 in qVersion){
			//writeLogEntry("deleteSchemaSetRecursively version set id:"&row2.site_x_option_group_set_id);
			deleteSchemaSetRecursively(row2.site_x_option_group_set_id);
		}
	}
	...
	//writeLogEntry("deleteSchemaSetIndex version set id:"&arguments.site_x_option_group_set_id);
	deleteSchemaSetIndex(arguments.site_x_option_group_set_id, request.zos.globals.id);
	...
	if(structkeyexists(groupStruct, 'feature_schema_change_cfc_path') and groupStruct.feature_schema_change_cfc_path NEQ ""){
		...
		changeCom[groupStruct.feature_schema_change_cfc_delete_method](arguments.site_x_option_group_set_id);
	}
	</cfscript>
</cffunction>
...
<cffunction name="deleteSchemaRecursively" localmode="modern" access="public" roles="member">
	...
	db.sql="SELECT * FROM #db.table("site_x_option_group_set", "jetendofeature")# 
	WHERE  site_x_option_group_set.feature_schema_id=#db.param(arguments.feature_schema_id)# and  
	site_x_option_group_set_deleted = #db.param(0)# and
	site_x_option_group_set.feature_id=#db.param(form.feature_id)#  ";
	qSets=db.execute("qSets");
	for(row in qSets){
	...
	for(row in qSchema){ 
		...
		if(row.feature_schema_subgroup_alternate_admin EQ 1){
			if(row.feature_schema_user_child_limit EQ 1){
				...
				qSet=db.execute("qSet"); 
				if(qSet.recordcount EQ 0){
					echo('<h2><a href="/z/feature/admin/features/userAddSchema?feature_id=0&site_x_option_group_set_id=&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Add #row.feature_schema_display_name#</a></h2>');
				}else{
					echo('<h2><a href="/z/feature/admin/features/userEditSchema?feature_id=0&site_x_option_group_set_id=#qSet.site_x_option_group_set_id#&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Edit #row.feature_schema_display_name#</a></h2>');
					echo('<ul>');
					for(row2 in qSchema){
						if(row2.feature_schema_parent_id EQ row.feature_schema_id){
							echo('<li><a href="/z/feature/admin/features/userManageSchema?feature_id=0&feature_schema_id=#row2.feature_schema_id#&site_x_option_group_set_parent_id=#qSet.site_x_option_group_set_id#">Manage #row.feature_schema_display_name#(s)</a></li>');
						}
					}
					echo('</ul>');
				}
			}else{
				echo('<h2><a href="/z/feature/admin/features/userManageSchema?feature_id=0&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Manage #row.feature_schema_display_name#(s)</a> | 
					<a href="/z/feature/admin/features/userAddSchema?feature_id=0&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Add #row.feature_schema_display_name#</a></h2>');
			//	echo('<li><a href="/z/feature/admin/features/userAddSchema?feature_id=0&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Add #row.feature_schema_display_name#(s)</a></li>');
			}
		}else{
			echo('<h2><a href="/z/feature/admin/features/userManageSchema?feature_id=0&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Manage #row.feature_schema_display_name#(s)</a> | 
			<a href="/z/feature/admin/features/userAddSchema?feature_id=0&feature_schema_id=#row.feature_schema_id#&site_x_option_group_set_parent_id=0">Add #row.feature_schema_display_name#</a></h2>');
	...
	if(arguments.action NEQ "deleted"){
		if(qSchema.feature_schema_enable_unique_url EQ 1){
			if(qCheckSet.site_x_option_group_set_override_url NEQ ""){
				...
				var urlId=request.zos.globals.optionSchemaUrlId;
				if(urlId EQ "" or urlId EQ 0){
					throw("feature_schema_url_id is not set for site_id, #site_id#.");
				}
		...
		ts.html&='<a href="#request.zos.globals.domain#/z/feature/admin/features/editSchema?feature_id=0&amp;feature_schema_id=#qCheckSet.feature_schema_id#&amp;site_x_option_group_set_id=#qCheckSet.site_x_option_group_set_id#&amp;site_x_option_group_set_parent_id=0" target="_blank">Edit</a>';
	}
...
<cffunction name="deleteNotUpdatedSchemaSet" localmode="modern" access="public" returnType="struct">
	...
	for(row in qSet){
		deleteSchemaSetRecursively(row.site_x_option_group_set_id, row);
		rs.deleteCount++;
	}
	return rs;
	</cfscript>
