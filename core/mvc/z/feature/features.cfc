...
<cffunction name="index" localmode="modern" access="remote" roles="member">
	...
	application.zcore.functions.zStatusHandler(request.zsid);
	request.zsession["siteoption_return"&application.zcore.functions.zso(form, 'feature_field_id')]=application.zcore.functions.zso(form, 'returnURL');		
	form.jumpto=application.zcore.functions.zso(form, 'jumpto');
	feature_schema_id=application.zcore.functions.zso(form, 'feature_schema_id',true);
   	db.sql="SELECT * FROM #db.table("feature_field", "jetendofeature")# feature_field 
	LEFT JOIN #db.table("site_x_option", "jetendofeature")# site_x_option ON 
	feature_field.feature_field_id = site_x_option.feature_field_id and 
	site_x_option.feature_id=#db.param(form.feature_id)# and 
	feature_field.site_id = #db.trustedSQL(application.zcore.functions.zGetSiteIdTypeSQL("site_x_option.feature_field_id_siteIDType"))# and 
	site_x_option.feature_id=#db.param(form.feature_id)# and 
	site_x_option_deleted = #db.param(0)#
	LEFT JOIN #db.table("feature_schema", "jetendofeature")# feature_schema ON 
	feature_schema.feature_schema_id = feature_field.feature_schema_id and 
	feature_schema_deleted = #db.param(0)# and
	feature_schema.site_id = site_x_option.site_id and 
	feature_schema.feature_id=#db.param(form.feature_id)# 
	WHERE feature_field.site_id IN (#db.trustedSQL(variables.publicSiteIdList)#) and 
	feature_field_deleted = #db.param(0)# and
	(feature_schema.feature_schema_id IS NULL or 
	feature_schema.feature_schema_type =#db.param('0')#) ";
	if(feature_schema_id EQ 0){
		db.sql&=" and feature_field.feature_schema_id=#db.param('0')#";
	}
	if(structkeyexists(request.zos,'listing') EQ false){
		db.sql&=" and feature_field_listing_only=#db.param('0')#";
	}
	db.sql&=" ORDER BY feature_schema.feature_schema_display_name ASC, feature_field.feature_field_name ASC ";
	qS=db.execute("qS");
	theTitle="Features";
	application.zcore.template.setTag("title",theTitle);
	application.zcore.template.setTag("pagetitle",theTitle);
	...
	if(qu9.recordcount NEQ 0 and form.feature_id EQ 0){
		...
		echo('<br /><br />
			<form class="zFormCheckDirty" action="/z/feature/admin/features/sendAutoresponderTest" method="get">
	...
	if(qS.recordcount EQ 0){
		...
		writeoutput('
			<h3>Click on an option name below to edit it</h3> <form  class="zFormCheckDirty" name="myForm" action="/z/feature/admin/features/saveFields?feature_id=#form.feature_id#" method="post" enctype="multipart/form-data">
			...
			for(row in qS){ 
				...
				if(not structkeyexists(form, "newvalue"&row.feature_field_id)){
					form["newvalue"&row.feature_field_id]=row.site_x_option_value;
					if(row.site_x_option_id EQ ""){
						form["newvalue"&row.feature_field_id]=row.feature_field_default_value;
					}
				}
				...
				var currentCFC=application.zcore.siteFieldCom.getTypeCFC(row.feature_field_type_id); 
				dataStruct=currentCFC.onBeforeListView(row, optionStruct[row.feature_field_id], form);
				value=currentCFC.getListValue(dataStruct, optionStruct[row.feature_field_id], application.zcore.functions.zso(form, "newvalue"&row.feature_field_id));
				if(value EQ ""){
					value=row.feature_field_default_value;
				}
				labelStruct[row.feature_field_name]=value;
			...
			for(row in arrRow){ 
				currentRowIndex++;
				if(lastSchema NEQ row.feature_schema_name){
					lastSchema = row.feature_schema_name;
					writeoutput('<tr>
						<td colspan="2"><h2>#htmleditformat(row.feature_schema_display_name)#</h2></td>
					</tr>');
				}
				writeoutput('<tr ');
				if(currentRowIndex MOD 2 EQ 0){
				...
				writeoutput('>
				<td style="vertical-align:top;" colspan="2" style="padding-bottom:10px;"><a id="soid_#row.feature_field_id#" style="display:block; float:left;"></a>
					<div style="padding-bottom:5px;float:left; width:99%;"><a href="##" class="soid_link" title="Click to toggle editing the value of this option." data-id="#row.feature_field_id#">#row.feature_field_display_name#</a> 
					<a href="##" onclick="document.myForm.submit();return false;" class="zSiteFieldSave" id="soid_save#row.feature_field_id#" style="display:none;">Save</a></div>
					<div id="soid_field#row.feature_field_id#" style="display:none;">
					<input type="hidden" name="feature_field_id" value="#row.feature_field_id#" />
			...
			writeoutput('<tr>
				<td>&nbsp;</td>
				<td><button type="submit" name="submitForm" class="z-manager-search-button">Save Fields</button>
					&nbsp;');
					if(structkeyexists(form,'returnURL') and form.returnURL NEQ ""){
...
<cffunction name="sendAutoresponderTest" localmode="modern" access="remote">
	...
	if(qCheck.recordcount){
		...
		db.sql="select * from #db.table("contact", "jetendofeature")# WHERE 
