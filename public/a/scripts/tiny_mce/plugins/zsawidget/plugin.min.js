...
tinymce.PluginManager.add('zsawidget', function(editor, url) {
	...
	editor.on('click', function (e) {
		...
		editor.windowManager.open({
			title: 'Insert Widget',
			url: url + '/dialog.htm',
			width: Math.round(Math.min(1920, window.innerWidth*.9)),
			height: Math.round(Math.min(1080, window.innerHeight*.8)),
			buttons: [ 
	...
	editor.addButton('deleteLayout', {
		...
		onclick: function () {  
			...
			while(currentElement){ 
				...
				currentElement=currentElement.parentNode;
				if(currentElement.nodeName == "HTML" || currentElement.nodeName == "#document"){
	...
	editor.addButton('moveRowDown', {
		...
		onclick: function () {  
			...
			if(lastElement.nextSibling){
				if(lastElement.nextSibling.nextSibling){
					var sibling=lastElement.nextSibling.nextSibling;
					var parent=lastElement.parentNode;
					parent.removeChild(lastElement);   
					parent.insertBefore(lastElement, sibling);   
				}else{
